...
if(isset($_POST['delete']) && isset($_POST['deleteId'])){
    foreach($_POST['deleteId'] as $deleteId){
            $sql="DELETE from groupe where codeGroupe = '$deleteId'";
...
<html lang="en">
  <head>
    <title>Suppression Des Groupes</title>
    <meta charset="UTF-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <link rel="stylesheet" href="SuppressionDesGroupes.css">
      ...
      <div class="home-section">
        ...
        <form class="" action="" method="POST">
          <table id="table">
              ...
              $sql="SELECT * from groupe order by annéé,nomgroupe ";
              $pdo_statement=$pdo_conn->prepare($sql);
              $pdo_statement->execute();
              ...
              ?>
              <tr class="tr">
                <td class="td-check"><input type='checkbox' name='deleteId[]' value='<?php echo $row['codeGroupe']?>'/></td>
                <td hidden class="td"><?php $i++; ?></td>
                <td class="td"><?php echo $row['nomGroupe']?></td>
                <td class="td"><?php echo $row['nomfiliere']?></td>
                <td class="td"><?php echo $row['Annéé']?></td> 
              </tr>
              <?php } ?>
          </table>
          <button type="submit" name="delete" id="Btn_Delete" onclick="return confirm('Etes-vous sûr de vouloir supprimer ce groupe !!')">Supprimer</button>
        </form>
      </div>
  </body>
</html>
