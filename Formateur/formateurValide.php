...
$pdo_statement = $pdo_conn->prepare("SELECT distinct codeGroupe FROM formateur where codeGroupe!='Default'");
//lancer l'éxecution de la requete préparé via la méthode execute;
$pdo_statement->execute();
// charger le résultat dans une variable de type array

$groupes2= $pdo_statement->fetchAll(PDO::FETCH_ASSOC);
      ...
      <form method="post" action="option.php">
          <div class="lol">
              <div class="sel" id="sele">
                  <ul class="ul">
                    <li>
                        <input type="text" placeholder="nom de Formateur"  name="formateur" id="Formateur" >
                        <input type="hidden" value="<?=$formateur?>" name="formateur2" >
                      </li>
                      <li class="li-btn-insert">
                      <a href="ajouter.php"><input   type="button" value="Ajouter Formateur" id="insert" class="btn_insert"></a></li>
                     </li>
                     <li class="li-btn-insert">
                      <a href="listformateur.php"><input   type="button" value="List Formateur" id="insert" class="btn_insert"></a></li>
                     </li>              
                  ...
                  <table id='tableGroupe'>
                    ...
                    <tbody>
                      <?php foreach($groupes as $row) {if($row['codegroupe']==="Default"){continue;}else{?>
                      <tr>
                        
                        <td id="C-group"><?php echo $row['codegroupe'] ?></td>

                  
                        <td id='S-group'><a href='supprimer.php?nomf=<?= $formateur ?>&codegroupe=<?= $row["codegroupe"]?>' onclick="return confirm('Etes vous sur de vouloir supprimer ce groupe!!')"><span class='remove' id='delete'><i class='fa-light fa-trash'></i></span></a></td>
                        </tr>
                        <?php } } ?>
                      <tr><td>
                        <select name="codeGroupe" id="codeGroupe"  class="select-ajouter" >
                         <option value="" required>Choisir Un Groupe</option>
                    <?php foreach($groupes2 as $row){?>
                    <option value="<?php echo $row['codeGroupe'] ?>"><?php echo $row['codeGroupe'] ?></option>
                   
        <?php
        } 
        ?>

          </select></td><td><input type="submit" name="Ajouter" value="Ajouter" id="Ajouter" class="btn_valider"></td></tr>
